...
app.get("/admin/metrics", (req, res,next: Function) => {
  try{res.status(200).header("Content-Type", "text/html; charset=utf-8").send(`<html>
  <body>
...
app.post("/admin/reset", (req, res,next: Function) => {
  try{
...
app.post("/api/validate_chirp", (req, res,NextFunction: Function) => {
  try{
  let body = ""; 
  req.on("data", (chunk) => {
    body += chunk;
  ...
  req.on("end", () => {
    try {
      ...
      const arr_words:string[] = parsedBody.body.split(" ")
